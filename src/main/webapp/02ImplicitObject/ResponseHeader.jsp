...
add계열의 메서드를 통해 응답헤더를 추가한다.
	addDateHeader(문자열, long형의 타임스탬프)
		...
		대한민국은 표준시 +09, 즉 0시간이 느리므로 09시로 설정해야 정상적인
...
*/
response.addHeader("myName", "안중근");//앞에서 설정한 값을 안중근으로 수정한다.


/*
파일 다운로드 구현시 사용하는 설정
	: 웹 브라우저가 인식하지 못하는 MIME 타입으로 설정하면 웹브라우저는
...
사이트에 접속할때마다 F5를 누른 것처럼 새로운 정보를 서버로부터 받아
갱신하게 된다.
*/
...
<body>
	...
	*/
	...
	<%
	/*
	myNubmer라는 헤더명으로 2개의 값을 추가했으므로 아래에서는 각각의 값이
