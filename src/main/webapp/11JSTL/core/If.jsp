...
<body>
	...
	<!-- 
		JSTL의 if태그는 else 구문이 따로 없으므로 두개의 if 태그를
		통해 else와 같이 사용해야 한다.
		만약 여러개의 조건을 사용할 경우라면 choose 태그를 사용하는게 좋다.
	 -->
	...
	<c:if test="${ not result2 }"><!-- result2의 not조건을 통해 else를 구현 -->
		'Java'가 아닙니다. <br />
	</c:if>
	...
	<c:if test= "${not empty param.user }">
		...
		<!-- 
		아이디, 패스워드의 일치 여부를 판단한 후 result에 결과를 저장ㅇ한다.
	...
	<%-- <!-- 내 답안. 정답! -->
	<c:if test="${empty param.user and empty param.pass }" var="result4">
		<br />
	</c:if>
	...
	<c:if test="${ (not result4)&&(not result5) }">
		"아이디와 비번을 확인하세요" <br />
	</c:if> --%>
