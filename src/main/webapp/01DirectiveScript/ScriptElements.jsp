...
<%--
	선언부(Declaration): 스트립트릿이나 표현식에서 사용할 메서드를
		선언할때 사용된다. 선언부에서 선언된 메서드는 해당jsp가
...
<body>
<%--
	스크립트릿(Scriptlet): 주로 JSP 코드를 작성하고 실행할때 사용하는
		영역이다. 스크립트릿은 body, head 어디서든 사용할 수 있다. 또한
		<script>, <style> 태그 내부에서도 사용할 수 있다.
		여기서 기술된 코드는 _jspService() 메서드 내에 포함된다. 
		그러므로 해당 영역에는 메서드를 선언할 수 없다.
	표현식(Expression): 변수를 웹브라우저상에 출력할 때 사용한다.
		JS의 document.write()와 동일한 역할을 한다. 표현식 사용시 주의할
		점은 문장 끝에 ;(세미콜론)을 쓰면 에러가 발생한다.
 --%>
<%
